...
export default function Home() {
  ...
  const renderFeatures = useMemo(() => {
    if (dep && prov) return geoDists.filter((f: any) => jneCodeForMap(f.properties.NOMBDEP) === dep && (() => { const p = territoryIndex.find((i:any) => i.level === "PROVINCIA" && i.dep === dep && norm(i.name) === norm(f.properties.NOMBPROV)); return p ? p.prov === prov : false; })());
    if (dep) return geoProvs.filter((f: any) => jneCodeForMap(f.properties.FIRST_NOMB) === dep);
    return geo;
  }, [dep, prov, geo, geoProvs, geoDists, territoryIndex]);

  useEffect(() => {
    let targetFeature: any = null;
    if (dep && prov && geoProvs.length) {
      targetFeature = geoProvs.find((f: any) => jneCodeForMap(f.properties.FIRST_NOMB) === dep && (() => { const p = territoryIndex.find((i:any) => i.level === "PROVINCIA" && i.dep === dep && norm(i.name) === norm(f.properties.NOMBPROV)); return p ? p.prov === prov : false; })());
    } else if (dep && geo.length) {
        ...
        <section className="national-content">
          ...
          <section className="national-map-card">
            ...
            <div className="geo-wrap">
              {geo.length ? (
                <svg viewBox={viewBox} role="img" aria-label="Mapa interactivo del Per�" style={{transition:"viewBox 0.5s ease-in-out"}}>{renderFeatures.map((f: any) => { let code="", name="", isSelected=false, onClick=()=>{}; if (dep && prov) {
                  const dTarget = territoryIndex.find((i:any) => i.level === "DISTRITO" && i.dep === dep && i.prov === prov && norm(i.name) === norm(f.properties.NOMBDIST));
                  code = dTarget ? dTarget.dist : "";
                  name = f.properties.NOMBDIST;
                  isSelected = dist === code;
                  onClick = () => { if(code) { setDist(code); setLevel("6"); } };
                } else if (dep) {
                  const p = territoryIndex.find((i:any) => i.level === "PROVINCIA" && i.dep === dep && norm(i.name) === norm(f.properties.NOMBPROV));
                  code = p ? p.prov : "";
